<template>
    <v-expansion-panels>
        ...
        <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="secondary" variant="text" @click="openDialog(request.request_id)" :disabled="request.status !== 'Pending'">
                Approve / Reject
            </v-btn>
        </v-card-actions>
        </v-expansion-panel>
  ...
  export default {
    ...
    methods: {
      async approveRequest(requestId) {
        try {
          await fetch(`${import.meta.env.VITE_BACKEND_URL}/approve_withdrawal_request`, {
            method: "PUT",
            headers: {
              "Content-Type": "application/json"
            },
            body: JSON.stringify({ 
                withdrawal_request_id: this.selectedRequestId,
      ...
      async rejectRequest() {
        try {
          await fetch(`${import.meta.env.VITE_BACKEND_URL}/reject_withdrawal_request`, {
            method: "PUT",
            headers: {
              "Content-Type": "application/json"
            },
            body: JSON.stringify({ 
              ...
              reason_for_status: this.rejectionReason 
            })
          });
          this.closeDialog();
          this.$emit("update-requests");
