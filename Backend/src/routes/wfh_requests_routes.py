...
from src.services.wfh_requests_services import get_manager_team, apply_wfh, can_apply_wfh, view_pending_wfh_requests, approve_wfh_request, reject_wfh_request, get_all, find_by_request_id, find_by_staff_id, find_by_team, delete_wfh_request
...
def get_team_by_reporting_manager(manager_id):
    team_data, status_code = get_manager_team(manager_id)
...
def apply_for_wfh_request():
    ...
    if not can_apply_wfh(staff_id, chosen_date):
        return jsonify({"error": "You have reached your WFH limit or have conflicting dates"}), 400
    
    # Call the service function
    response, status_code = apply_wfh(staff_id, reporting_manager, dept, chosen_date, arrangement_type, request_datetime, status, remarks)
...
def get_pending_wfh_requests_of_own_team():
    ...
    # Call the service function
    requests_data, status_code = view_pending_wfh_requests(manager_id)
...
def approve_pending_wfh_request():
    ...
    response, status_code = approve_wfh_request(request_id, manager_id)
    return jsonify(response), status_code
...
def reject_pending_wfh_request():
    ...
    response, status_code = reject_wfh_request(request_id, rejection_reason)
    return jsonify(response), status_code
...
def get_wfh_request_by_request_id(request_id_num):
    wfh_request = find_by_request_id(request_id_num)
...
def get_wfh_requests_by_staff_id(staff_id_num):
    staff_requests_list = find_by_staff_id(staff_id_num)
...
def get_wfh_requests_by_team(reporting_manager_id_num):
    team_requests_list = find_by_team(reporting_manager_id_num)
...
def withdraw_request_by_id(request_id_num):
    status_code, error_message = delete_wfh_request(request_id_num)
